            ...
            try:
                ...
                if btn:
                    ...
                    await message.reply_photo(
                        ...
                        caption=f"👋 ʜᴇʟʟᴏ {message.from_user.mention},\n\nᴘʟᴇᴀsᴇ ᴊᴏɪɴ ᴛʜᴇ ᴄʜᴀɴɴᴇʟ ᴛʜᴇɴ ᴄʟɪᴄᴋ ᴏɴ ᴜɴᴍᴜᴛᴇ ᴍᴇ ʙᴜᴛᴛᴏɴ. 😇",
        ...
        if manual == False:
            ...
            try:
                if settings['auto_ffilter']:
                    try:
                        print("Auto filter is ON")
                        if re.search(r'https?://\S+|www\.\S+|t\.me/\S+', message.text):
                            print("Detected link in message")
                            if await is_check_admin(client, message.chat.id, message.from_user.id):
                                print("User is admin — skipping")
                                return
                            print("Deleting link message")
                            await client.send_message(
                                chat_id=LOG_CHANNEL,
                                text=script.DEL_LNK_TXT.format(message.from_user.mention, message.from_user.id, message.chat.title, message.chat.id, message.text, temp.U_NAME, temp.B_NAME),
                                parse_mode=enums.ParseMode.HTML
                            )
                            return await message.delete()
                        ai_search = True
                        reply_msg = await message.reply_text(f"<b>𝐒ᴇᴀʀᴄʜɪɴɢ 𝐅ᴏʀ {message.text} 🔍</b>")
                        await auto_filter(client, message.text, message, reply_msg, ai_search)
                        await client.send_message(
                            chat_id=LOG_CHANNEL,
                            text=script.REQALART_LNK_TXT.format(message.from_user.mention, message.from_user.id, message.chat.title, message.chat.id, message.text, temp.U_NAME, temp.B_NAME),
                            parse_mode=enums.ParseMode.HTML
                        )
                    except Exception as e:
                        print(f"Error in auto_ffilter block: {e}")
            except KeyError:
                ...
                if settings['auto_ffilter']:
                    try:
                        ...
                        print(f"Error in auto_ffilter block (key error fallback): {e}")
    else:
        search = message.text
    ...
    if gl == False:
        ...
        if k == False:
            ...
            if files:
                ...
                ai_search = True
                reply_msg = await query.message.edit_text(f"<b><i>𝐒ᴇᴀʀᴄʜɪɴɢ 𝐅ᴏʀ {movie} 🔍</i></b>")
    ...
    if query.data.startswith("file"):
     ...
     #   if not search:
      ...
      #      return
        ...
        buttons = [[
            InlineKeyboardButton('♻️ sᴛᴀᴛs ♻️', callback_data='stats'),
        ...
        buttons = [[
            InlineKeyboardButton('ꜱᴏᴜʀᴄᴇ ᴄᴏᴅᴇ 📜', url='https://github.com/SMDxTG/AutoFilterBot.git'),
            InlineKeyboardButton('⇋ ʙᴀᴄᴋ ⇋', callback_data='about')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        await query.answer("⚡️ sᴛᴀᴛᴜs ⚡️\n\n❂ ʀᴀᴍ ●●●●●●●◌◌◌\n✇ ᴄᴘᴜ ●●●●●●●◌◌◌\n✪ ᴅᴀᴛᴀ ᴛʀᴀꜰɪᴄs ●●●●◌◌◌◌◌◌ 🛰\n\n[sᴛᴀʙʟᴇ] """, show_alert=True)
        ...
        await query.answer("ғᴇᴛᴄʜɪɴɢ sᴍᴅ ᴍᴏɴɢᴏᴅʙ ᴅᴀᴛᴀʙᴀsᴇ")
        buttons = [[
            InlineKeyboardButton('⇇ ʙᴀᴄᴋ', callback_data='about'),
            InlineKeyboardButton('⟲ ʀᴇғʀᴇsʜ', callback_data='rfrsh')
        ]]
        ...
        btn = [[
            ...
            InlineKeyboardButton("ʜᴇʟᴘ ᴜᴘ", url=OWNER_LNK)
        ],[
            InlineKeyboardButton("⇋ ʙᴀᴄᴋ ⇋", callback_data="start")
        ]]
        await client.edit_message_media(
...
async def auto_filter(client, name, msg, reply_msg, ai_search, spoll=False):
    ...
    if imdb:
        ...
        if settings["button"]:
            cap = f"<b>ᴛɪᴛᴛʟᴇ {search}\n\nʀᴇǫᴜᴇsᴛᴇᴅ ʙʏ {message.from_user.mention}\n\nʀᴇsᴜʟᴛ sʜᴏᴡ ɪɴ {remaining_seconds} sᴇᴄᴏɴᴅs\n\nᴘᴏᴡᴇʀᴇᴅ ʙʏ : {message.chat.title} \n\n⚠️ ᴀꜰᴛᴇʀ 5 ᴍɪɴᴜᴛᴇꜱ ᴛʜɪꜱ ᴍᴇꜱꜱᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ ᴅᴇʟᴇᴛᴇᴅ 🗑️\n\n</b>"
        else:
            cap = f"<b>ᴛɪᴛᴛʟᴇ {search}\n\nʀᴇǫᴜᴇsᴛᴇᴅ ʙʏ {message.from_user.mention}\n\nʀᴇsᴜʟᴛ sʜᴏᴡ ɪɴ {remaining_seconds} sᴇᴄᴏɴᴅs\n\nᴘᴏᴡᴇʀᴇᴅ ʙʏ : {message.chat.title} \n\n⚠️ ᴀꜰᴛᴇʀ 5 ᴍɪɴᴜᴛᴇꜱ ᴛʜɪꜱ ᴍᴇꜱꜱᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ ᴅᴇʟᴇᴛᴇᴅ 🗑️\n\n</b>"
            cap+="<b><u>🍿 Your Movie Files 👇</u></b>\n\n"
            for file in files:
                cap += f"<b>📁 <a href='https://telegram.me/{temp.U_NAME}?start=files_{file['file_id']}'>[{get_size(file['file_size'])}] {' '.join(filter(lambda x: not x.startswith('['), file['file_name'].split()))}\n\n</a></b>"
...
async def advantage_spell_chok(client, name, msg, reply_msg, vj_search):
    ...
    if AI_SPELL_CHECK == True and vj_search == True:
        vj_search_new = False
        vj_ai_msg = await reply_msg.edit_text("<b><i>ɪ ᴀᴍ ᴛʀʏɪɴɢ ᴛᴏ ғɪɴᴅ ʏᴏᴜʀ ᴍᴏᴠɪᴇ ᴡɪᴛʜ ʏᴏᴜʀ ᴡʀᴏɴɢ sᴘᴇʟʟɪɴɢ.</i></b>")
        movienamelist = []
